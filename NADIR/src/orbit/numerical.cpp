...
CartesianState derivative(const CartesianState& state, const GravityModel& model) {
    return {state.velocity_m_s, gravity_acceleration(state.position_m, model)};
}
...
math::Vec3d gravity_acceleration(const math::Vec3d& r, const GravityModel& model) {
    ...
    return {base * r.x * (1.0 + j2_factor * (1.0 - 5.0 * z2_over_r2)),
...
std::optional<CartesianState> propagate_numerical(CartesianState state, double duration_s,
    ...
    while (remaining > 0.0) {
        ...
        const auto k1 = derivative(state, model);
        const auto k2 = derivative(add_scaled(state, k1, h * 0.5), model);
        const auto k3 = derivative(add_scaled(state, k2, h * 0.5), model);
        const auto k4 = derivative(add_scaled(state, k3, h), model);
